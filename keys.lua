...
keys.globalkeys = gears.table.join(
    ...
    awful.key({ superkey,shiftkey }, "d",
        function()
            awful.spawn.with_shell("rofi -modi drun,run -show drun -show-icons")
        end,
        {description = "rofi launcher", group = "launcher"}),
    -- Run
    ...
    -- Spawn audio mixer
    awful.key({ superkey }, "F4", function() awful.spawn(user.terminal .. " -e 'pulsemixer'") end,
        {description = "mixer", group = "launcher"}),
    -- Spawn music client
    awful.key({ superkey }, "F3", apps.music,
        {description = "music client", group = "launcher"}),
    -- Spawn cava in a terminal
    awful.key({ superkey, shiftkey }, "F4", function() awful.spawn("visualizer") end,
    ...
    awful.key({ superkey }, "o", function() awful.spawn("emacsclient -c -a 'emacs'") end,
        {description = "emacs", group = "launcher"}),
    -- Thunderbird Email client   
    awful.key({ superkey, ctrlkey }, "e", apps.mail,
        {description = "thunderbird", group = "mail"}),
   -- Firefox browser
     awful.key({ superkey, shiftkey }, "b", apps.browser,
        {description = "firefox", group = "browser"}),
  -- Brave
     awful.key({ superkey, ctrlkey }, "b",function() awful.spawn("brave") end,
...
keys.clientkeys = gears.table.join(
    ...
    -- Move client to next screen (cycling)
    awful.key({ superkey, ctrlkey }, "b",
    function (c)
        c:move_to_screen(s)
    end,
